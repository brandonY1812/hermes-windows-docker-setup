...
while true; do
  ...
  if [ -n "$FOUND" ]; then
    # Determine if we need to (re)start x11vnc
    NEEDS_START=false
    if [ "$FOUND" != "$CURRENT_DISPLAY" ]; then
      # Display changed -- kill old x11vnc, start new one
      NEEDS_START=true
    ...
    elif [ -z "$X11VNC_PID" ] || ! kill -0 "$X11VNC_PID" 2>/dev/null; then
      # Same display but x11vnc died (Xvfb went away and came back)
      NEEDS_START=true
      log "x11vnc died (was pid=$X11VNC_PID), re-attaching to DISPLAY=$FOUND"
    fi
